...
export default class Sctw_IconsRow extends LightningElement {
  ...
  get hasFirstIcon() {
    return this.firstIcon && this.firstIcon !== 'None';
  }

  get hasSecondIcon() {
    return this.secondIcon && this.secondIcon !== 'None';
  }

  get hasThirdIcon() {
    return this.thirdIcon && this.thirdIcon !== 'None';
  }

  renderedCallback() {
    const firstIconWrapper = this.template.querySelector('.first-icon-wrapper');
    if (this.hasFirstIcon && firstIconWrapper) {
      insertIconIntoElement(this.firstIcon, firstIconWrapper);
    }
    const secondIconWrapper = this.template.querySelector(
      '.second-icon-wrapper'
    );
    if (this.hasSecondIcon && secondIconWrapper) {
    ...
    if (this.hasThirdIcon && thirdIconWrapper) {
